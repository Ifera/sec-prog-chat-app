...
async def lifespan(app: FastAPI):
    ...
    if not config.is_introducer:
        # Give the server a moment to start up before bootstrapping
        ...
        for introducer in config.bootstrap_servers:
            ...
            try:
                ...
                if e.status_code == 403:
                    logger.warning(f"Access forbidden (403) - make sure introducer is running on {introducer['host']}:{introducer['port']}")
                else:
                    logger.warning(f"WebSocket error {e.status_code}: {e}")
